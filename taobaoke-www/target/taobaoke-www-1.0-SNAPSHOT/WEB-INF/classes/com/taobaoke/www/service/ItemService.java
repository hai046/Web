...
public class ItemService {
    ...
    public List<TItem> getWebItemList(long connectId){
        List<Long> itemList = null;
        try {
            itemList = WebItemHome.getInstance().getItemList(connectId);
        } catch (SQLException e) {
            e.printStackTrace();
        }
...
//        JSONArray result = new JSONArray();
        ...
        for( Long numIid : itemList ){
            try {
                ...
                if(tItem == null){
                    WebItemHome.getInstance().delete(connectId, numIid, WebItemHome.TYPE_ITEM);
                    continue;
                }
