package com.taobaoke.api.crawler;
...
public class ItemDetailCrawler {
	...
	public ItemDetailCrawler() {
	}

	public static TItemAllDetail saveTaobaokeItem(long numIid)
			throws SQLException {
		TaobaoClient client = new DefaultTaobaoClient(TaobaokeConstants.url,
				TaobaokeConstants.appkey, TaobaokeConstants.appSecret);
...
//			dfa.getItem()
			...
			TItemDescHome.getInstance().insert(tItemDetail.getNumIid(),
					item.getDesc());
			mItemAllDetail.setDesc(item.getDesc());
			for (ItemImg itemImg : item.getItemImgs()) {
				TItemImg tItemImg = changeItemImg2TItemImg(itemImg,
						tItemDetail.getNumIid());
